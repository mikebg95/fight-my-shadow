...
/// The canonical list of all 39 Boxing moves in the app.
///
/// This serves as the single source of truth for Boxing moves.
///
/// Structure:
/// - Moves 1-14: Punches (numeric codes 1-14)
/// - Moves 15-28: Defense (letter codes A-M, N, O)
/// - Moves 29-38: Footwork (letter codes P-Y)
/// - Move 39: Deception (letter code F)
///
/// Each move has:
...
final List<Move> boxingMoves = [
  ...
  const Move(
    ...
    category: MoveCategory.defense,
    discipline: Discipline.boxing,
    description: 'Using your left elbow to block straight punches or uppercuts aimed at your centerline/solar plexus. Brings your left elbow inward to cover your body.',
  ),
  const Move(
    id: 28,
    code: 'O',
    ...
    category: MoveCategory.defense,
    discipline: Discipline.boxing,
    description: 'Using your right elbow to block straight punches or uppercuts aimed at your centerline. Protects against body shots to the midsection.',
  ),

  // ========== FOOTWORK (LETTERS P-Z) ==========
  const Move(
    id: 29,
    code: 'P',
    name: 'Step in',
  ...
  const Move(
    ...
    discipline: Discipline.boxing,
    description: 'A deceptive movement that mimics the start of a strike without fully committing. Feints draw reactions from your opponent, create openings, and disrupt their rhythm.',
    tips: [
      'Make it look real - sell the fake',
      'Watch for their reaction',
      'Create openings for your real attacks',
      'Can be combined with any punch',
    ],
  ),
];
...
/// final defense = getMovesByCategory(MoveCategory.defense);    // Returns moves A-M, N, O
/// final footwork = getMovesByCategory(MoveCategory.footwork);  // Returns moves P-Y
/// final deception = getMovesByCategory(MoveCategory.deception); // Returns move F
/// ```
...
///
/// This is the canonical ordering used throughout the app:
/// - Library screens
/// - Academy progression screens
/// - Training session move selectors
/// - Any move pickers or dropdowns
///
/// The ordering follows the Academy's 12 levels, with moves appearing
/// in the exact sequence they are taught in the Academy curriculum.
/// Moves not part of the Academy (Circle left/right) appear at the end.
List<Move> getMovesInAcademyOrder() {
  // Import needed to access LearningPath
  // This function returns moves in Academy order: Level 1-12 moves first,
  // then any remaining moves (Circle left X, Circle right Y) at the end

  final academyCodes = [
    // Level 1: Foundation Fists
    '1', '2', 'P', 'Q',
    // Level 2: Rails & Shields
    'R', 'S', 'G', 'I',
    // Level 3: Hook City
    '3', '4', 'J', 'K',
    // Level 4: Turn the Corner
    'T', 'U',
    // Level 5: Uppercut Engine
    '5', '6',
    // Level 6: Slipstream
    'A', 'B', 'E',
    // Level 7: Wave & Fade
    ...
    // Level 8: Shield Wall
    'M', 'L', 'N', 'O',
    // Level 9: Deception Arts
    '11', 'F',
    // Level 10: Footwork Fury
    'V', 'W',
    // Level 11: Body Hunters
    '9', '10', '7', '8',
    // Level 12: Shadow Weapons
    '12', '13', '14',
  ];

  final orderedMoves = <Move>[];

  // Add Academy moves in order
  for (final code in academyCodes) {
    final move = getMoveByCode(code);
    if (move != null) {
      orderedMoves.add(move);
    }
  }

  // Add any remaining moves not in Academy (Circle left X, Circle right Y)
  for (final move in boxingMoves) {
